...
|  No safari just did not work for me |  Tried passing through a safari test environment and could find the same bug | Asked user to try again to see if problem persisting – didn’t happen again but will keep tabs   |  :question: Still ongoing |   
...
|  With the comments, what is the purpose of me putting a comment on my own dog? Is this intentional? | Comments are allowed for both guests and profile  |  Add helper text to also reassure profile owner that they can comment on own profile with updates | :heavy_check_mark:   |   
